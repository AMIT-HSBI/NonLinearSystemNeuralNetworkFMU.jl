...
profilingInfos = getProfilingInfo(joinpath(workdir, "profilingInfo.bson"))

for (i,prof) in enumerate(profilingInfos)
  ...
  n_inputs = length(prof.usingVars)
  n_outputs = length(prof.iterationVariables)

  X = transpose(Matrix{Float32}(data[:,1:n_inputs]))
  Y = transpose(Matrix{Float32}(data[:,n_inputs+1:n_inputs+n_outputs]))

  # Symbolic Regression
  bestEq = fitEquation(X, Y; tempDir=joinpath(workdir, "temp-symbolicRegression_$(eq)"))

  surrogatEqns = genMoCode(bestEq, prof.usingVars, prof.iterationVariables)
  println.(surrogatEqns);

  surrogatEqnsMo = joinpath(workdir, "surrogatEqns_$(eq).mo")
