package JetPumpTool "Static relations for sizing of jet pump geometry"
  ...
  package Components
    ...
    equation
      // Replaced by smybolic surrogate
      // p_0/p = (1 + (k - 1)/2*Ma^2)^(k/(k - 1)) "eq. 85";
      // T_0/T = (1 + (k - 1)/2*Ma^2) "eq. 86";
      p = -2821.8457 + (1.0532645 * p_0) + (-1 * T_0 * (-1.2804059 + sqrt(mflow) + (5935.4814 * mflow)));
      T = 0.049019717 + T_0 + (T_0 * (mflow ^ 2) * (-347.94598 + (-1 * (T_0 + (0.0025247089 * (T_0 + (-1 * p_0)))))));
